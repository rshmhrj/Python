...
def lucky_search(index, ranks, keyword):
    ...
    if result:
        return get_ranks(result,ranks)
    else:
        return None
        
def get_ranks(result, ranks):
    highest = 0
    highestPageName = ''
    for page in result:
        #print str(page) + ": " + str(ranks[page])
        highest,highestPageName = get_highest(highest,ranks[page],highestPageName,page)
    return highestPageName

def get_highest(h,c,hp,cp):
    if c > h:
        return c,cp
    else:
        return h,hp
