...
def ancestors(genealogy, person):
    ...
    for people in genealogy:
        if person in people:
            result.append(genealogy[people][0])
            result.append(genealogy[people][1])
    for parents in result:
        if ancestors(genealogy, parents):
            result.append(ancestors(genealogy, parents)[0])
            result.append(ancestors(genealogy, parents)[1])
    return result
